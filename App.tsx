...
function App() {
  ...
  const modeTabClass = (target: AppMode) =>
    `flex-1 rounded border px-1.5 py-3 text-[11px] font-bold transition min-h-[44px] sm:px-2 sm:text-xs md:min-h-0 md:py-2 ${
      mode === target
  ...
  return (
    <div className="flex h-[100dvh] flex-col overflow-hidden bg-slate-50 text-slate-800 md:flex-row">
      ...
      <aside className="hidden h-full w-96 shrink-0 flex-col overflow-hidden border-r border-gray-200 bg-white shadow-sm md:flex">
        <div className="space-y-3 border-b border-gray-200 bg-gradient-to-br from-teal-50 to-white p-4">
          <h1 className="text-xl font-bold text-gray-800">よりそい Pro</h1>
          {modeTabs}
          <p className="text-sm text-gray-500">{MODE_DESCRIPTION[mode]}</p>
        </div>
        {mode === 'recording' && (
          <div className="flex-1 space-y-3 overflow-y-auto p-4 text-sm text-gray-600">
